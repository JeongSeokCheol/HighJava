...
/*
   Thread의 stop()메서드를 호출하면 쓰레드가 바로멈춘다.
    이 때 사용하던 자원을 정리하지 못하고 프로그램이 종요되어 나중에 실행되는 프로그램에 영향을 줄 수있다.
...
class ThreadStopTest2 extends Thread{
	@Override
	public void run() {
		//방법1 ==> interrupt()메서드의 sleep()메서드를 이용하는 방법
		/*try {
			while(true) {
				System.out.println("쓰레드 실행중..");
				Thread.sleep(1);//일시 정지 상태에서 interrupt()메서드가 호출되면
		...
		while(true) {
			...
			*/
			...
			//     interrupted()메서드로 interrupt()메서드가 호출되면 true를 반환한다.
